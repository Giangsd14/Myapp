...
@router.post("/{map_id}", response_model=schemas.ShowPoint)
async def create_point(db: db_depend, map_id: int, data: schemas.CreatePoint, get_current_user: current_user):
    return await point.create_point(db, map_id, data, get_current_user)
...
async def update_point(db: db_depend, map_id: int, point_id: int, data: schemas.CreatePoint, get_current_user: current_user):
